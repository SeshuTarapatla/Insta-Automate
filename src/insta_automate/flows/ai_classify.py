...
from insta_automate.tasks.ollama import access_classify, gender_classify
...
def ai_classify():
    ...
    if images:
        ...
        access_classify(session)
        gender_classify(session)
        time_taken = Timestamp() - started_at
        log.info(
            f"AI classification complete. Total entities processed: {total} at {total / (time_taken.total_seconds() or 1):.3} img/s rate. Time taken: {time_taken}"
        )
    else:
